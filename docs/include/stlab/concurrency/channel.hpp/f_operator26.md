...
hyde:
  ...
  inline:
    brief:
      - "!std::is_enum_v <std ::remove_reference_t <F >>, int> = 0 is used to exclude enum types from operator & so built-in & is used (e.g. doctest assertType)."
  ...
  overloads:
    ...
    "template <typename F, std::enable_if_t<!std::is_enum_v<std::remove_reference_t<F>>, int> >\nauto operator&(F &&, detail::annotations &&) -> detail::annotated_process<F>":
      arguments:
        - description: __OPTIONAL__
          name: f
          type: F &&
      ...
      description: __MISSING__
      return: __OPTIONAL__
      signature_with_names: "template <typename F, std::enable_if_t<!std::is_enum_v<std::remove_reference_t<F>>, int> >\nauto operator&(F && f, detail::annotations && a) -> detail::annotated_process<F>"
